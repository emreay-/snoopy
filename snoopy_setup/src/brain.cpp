...
public:
    ...
    //arm client vars
    snoopy_uarm::ObjectPosition arm_srv;
    ServiceClient arm_cli;
    ...
    action("navigate",true)
{
    ...
    //arm client
    arm_cli = n.serviceClient<snoopy_uarm::ObjectPosition>("uarm_do");
...
//                                    bool sceneTime = (Time::now()-the_brain.obj_detection_time).toSec() > 6;
        ...
        //                        snoopy_navigate::navigateResult res = *(the_brain.action.getResult());
                            ...
                            {
                                ROS_WARN("_brain_: ASTAR SRV FAILED");
                                break;
                            }
                        ...
                        {
                            ROS_WARN("_brain_: OGM GETTER FAILED");
                            break;
                        }
                    ...
                    {
                        ROS_WARN("_brain_: POSE SRV FAILED");
                        break;
                    }
                ...
                {
                    ROS_WARN("_brain_: EXPLORE GOAL CANT RECEIVED");
                    break;
        //            return 1;
                }
...
//                    cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!IF!IF!IF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
//                    cout << "OBJECT X,Y" << the_brain.obj_pose.point.x << " " << the_brain.obj_pose.point.y << endl;

                    ROS_ERROR("_brain_:OBJECT SHAPE %s",the_brain.object_type.data.c_str());
                    ROS_ERROR("_brain_:OBJECT POSE (X,Y)= (%f,%f) ",the_brain.obj_pose.point.x, the_brain.obj_pose.point.y );
...
//                    the_brain.state = ST_EXPLORE;
                    ...
                    else the_brain.state = ST_STOP_OBJECT;
            ...
            case ST_ARM_PICK:
                ...
                float x = (float) 100*(the_brain.obj_pose.point.x);
                float y = (float) 100*(the_brain.obj_pose.point.y+0.005);
                ROS_ERROR("_brain_:ARM GOAL(X,Y)= (%f,%f) ",x,y );
                the_brain.arm_srv.request.x = x;
                the_brain.arm_srv.request.y = y;
                the_brain.arm_srv.request.z = -16.0;

//                the_brain.arm_srv.request.x = 25.322;
//                the_brain.arm_srv.request.y = -1.5;
//                the_brain.arm_srv.request.z = -16.0;

                if(the_brain.arm_cli.call(the_brain.arm_srv))
                {
                    ROS_INFO("UARM SERVICE CALLED");
                }
                else{ROS_INFO("FAILED TO CALL UARM SERVICE");}
                sleep(10);
                the_brain.state = ST_STOP_OBJECT;
