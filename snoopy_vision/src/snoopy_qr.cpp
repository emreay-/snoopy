...
// Program : QR Code tracking
// Author  : Abhishek Choudhary
// (Original from Bharath Prabhuswamy @https://github.com/bharathp666/opencv_q )
//______________________________________________________________________________________
...
void qrCallback(const sensor_msgs::ImageConstPtr& input)
{
    //Convert from the ROS image message to a CvImage suitable for working with OpenCV for processing
    cv_bridge::CvImagePtr image_ptr;
    try
    {
        image_ptr = cv_bridge::toCvCopy(input, enc::BGR8);
    }
...
//            {
//                row_temp += (L[i].y + M[i].y + O[i].y);
//                col_temp += (L[i].x + M[i].x + O[i].x);
//            }

            row = (int) (L[0].y + M[0].y + O[0].y )/3;
            col = (int) (L[0].x + M[0].x + O[0].x )/3;
            index.x = col;
            index.y = row;
            circle( image, index, 4,  Scalar(255,255,0), -1, 8, 0 );
//            ROS_INFO("Row: %d Column: %d",row,col);
            ...
            if(DBG==1)
            {
                ...
                circle( traces, L[0], 2,  Scalar(255,255,255), -1, 8, 0 );
    ...
    waitKey(1);	// OPENCV: wait for 1ms before accessing next frame
}
...
//    ROS_INFO("HeightNAN: %d WidthNAN: %d", cloudNanRemoved->height, cloudNanRemoved->width );
//    ROS_INFO("Height: %d Width: %d", input->height, input->width );

    PointXYZRGB point = input->at(col,row);
    if(isFinite(point))
    {
        float x = point.x;
        float y = point.y;
        float z = point.z;
        ROS_INFO("_QR CODE_: Distance from Snoopy %f cm", 100*x);
    }
}
...
{
    init(argc, argv, "qr_detector");
    NodeHandle nh;

    image_transport::ImageTransport it(nh);
    image_transport::Publisher pub = it.advertise("snoopy/qr_code", 1);
    image_transport::Subscriber sub = it.subscribe("camera/rgb/image_raw", 1, qrCallback);
    
//    Subscriber sub_cloud = nh.subscribe<sensor_msgs::PointCloud2>("/camera/depth_registered/points",1,cloudCallback);
    Subscriber sub_cloud = nh.subscribe<PointCloud<PointXYZRGB> >("/snoopy/pcl_converted",1,cloudCallback);
